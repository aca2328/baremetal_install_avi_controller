...
docker run -v "$PWD/id_rsa_priv:/ansible/playbooks/id_rsa_priv:ro" -v "$PWD/hosts:/ansible/playbooks/hosts:ro" -v "$PWD/bare_install.yaml:/ansible/playbooks/bare_install.yaml:ro" --rm -ti avibuilder ansible-playbook --private-key=id_rsa_priv -b -u root -i hosts bare_install.yaml -e "ansible_python_interpreter=/usr/bin/python"
